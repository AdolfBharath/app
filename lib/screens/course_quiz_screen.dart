...
class _CourseQuizScreenState extends State<CourseQuizScreen> {
  ...
  bool get _completed => widget.initialState['completed'] == true;
  bool get _locked =>
  ...
  int get _passScore {
    ...
    return configured > 0 ? configured : 9;
  }

  List<CourseQuizQuestion> get _allQuestions {
    return widget.course.modules
        .expand((module) => module.quizQuestions)
  ...
  void _startAttempt() {
    final all = _allQuestions.toList(growable: true)..shuffle();
    setState(() {
      _questions = all.take(10).toList(growable: false);
      _answers.clear();
      _index = 0;
      _score = 0;
      _submitted = false;
  ...
  void _closeWithResult() {
    Navigator.of(context).pop(_savedResult);
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      } else if (result['quiz_locked'] == true) {
        showTopNotification(
          context,
          'Quiz locked after 5 attempts. Rewatch course lessons to retry.',
        );
      }
      _savedResult = result;
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final allQuestions = _allQuestions;
    final canAttempt = allQuestions.length >= 10 && !_locked;
          ...
          children: [
            ...
            if (allQuestions.length < 10)
              const _NoticeCard(
                ...
                text:
                    'The mentor needs to add at least 10 questions before students can attempt the final course quiz.',
              )
            else if (_locked)
              const _NoticeCard(
                icon: Icons.lock_outline_rounded,
                title: 'Course quiz locked',
                text:
                    'You have reached the course quiz attempt limit. Rewatch lessons before trying again.',
                isError: true,
              )
            else if (canAttempt)
              _submitted
...
class _CourseQuizHero extends StatelessWidget {
  const _CourseQuizHero({
    required this.course,
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      completed ? 'Final quiz completed' : 'Final course quiz',
          ...
          Wrap(
            ...
            children: [
              ...
              _StatusPill(
                ...
                label: '$attempts/5 attempts',
                color: scheme.onPrimary,
                onColor: scheme.primary,
              ),
              _StatusPill(
                icon: Icons.error_outline_rounded,
                label: '$failedAttempts failed',
                color: scheme.onPrimary,
                onColor: scheme.primary,
              ),
            ],
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      passed ? 'Course quiz passed' : 'Try again',
                      style: theme.textTheme.titleMedium?.copyWith(
                        fontWeight: FontWeight.w800,
                      ),
                    ),
